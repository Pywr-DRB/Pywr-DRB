...
for L in L_range:
    ccm_s_r = ccm(norm_s.flatten(), norm_r.flatten(), tau, E, L) # define new ccm object # Testing for X -> Y
    ccm_r_s = ccm(norm_r.flatten(), norm_s.flatten(), tau, E, L) # define new ccm object # Testing for Y -> X
...
shat_Mr, rhat_Ms= [], [] # correlation list
for t in t_range:
    ccm_s_r = ccm(norm_s.flatten(), norm_r.flatten(), t, E, L) # define new ccm object # Testing for X -> Y
    ccm_r_s = ccm(norm_r.flatten(), norm_s.flatten(), t, E, L) # define new ccm object # Testing for Y -> X
    shat_Mr.append(ccm_s_r.causality()[0])
    rhat_Ms.append(ccm_r_s.causality()[0])
...
shat_Mr, rhat_Ms= [], [] # correlation list
for L in L_range:
    ccm_s_r = ccm(resops_storage.flatten(), resops_outflow.flatten(), tau, E, L) # define new ccm object # Testing for X -> Y
    ccm_r_s = ccm(resops_outflow.flatten(), resops_storage.flatten(), tau, E, L) # define new ccm object # Testing for Y -> X
    shat_Mr.append(ccm_s_r.causality()[0])
    rhat_Ms.append(ccm_r_s.causality()[0])
...
shat_Mr, rhat_Ms= [], [] # correlation list
for L in L_range:
    ccm_s_r = ccm(norm_s.flatten(), np.log(resops_outflow).flatten(), tau, E, L) # define new ccm object # Testing for X -> Y
    ccm_r_s = ccm(np.log(resops_outflow).flatten(), norm_s.flatten(), tau, E, L) # define new ccm object # Testing for Y -> X
    shat_Mr.append(ccm_s_r.causality()[0])
    rhat_Ms.append(ccm_r_s.causality()[0])
...
shat_Mr, rhat_Ms= [], [] # correlation list
for t in t_range:
    ccm_s_r = ccm(norm_s.flatten(), np.log(resops_outflow).flatten(), t, E, L) # define new ccm object # Testing for X -> Y
    ccm_r_s = ccm(np.log(resops_outflow).flatten(), norm_s.flatten(), t, E, L) # define new ccm object # Testing for Y -> X
    shat_Mr.append(ccm_s_r.causality()[0])
    rhat_Ms.append(ccm_r_s.causality()[0])
...
ccm_mapping = ccm(norm_s.flatten()[90:], np.log(resops_outflow).flatten()[90:], tau, 2, L)
ccm_mapping.visualize_cross_mapping()
ccm_mapping.causality()
ccm_mapping.plot_ccm_correls()
