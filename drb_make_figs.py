...
scenario = 0
def get_pywr_results(output_dir, model):
...
for model in pywr_models:
    results[f'pywr_{model}'] = get_pywr_results(output_dir, model)
...
def get_base_results(input_dir, model, datetime_index):
...
datetime_index = list(results.values())[0].index
for model in base_models:
    results[model] = get_base_results(input_dir, model, datetime_index)
...
def plot_3part_flows(results, models, node, colors=['0.5', '#67a9cf', '#ef8a62'], uselog=False):
    ...
    for i, m in enumerate(models):
        if use2nd or i == 0:
            ...
            if m in pywr_models:
                try:
                    ### for reservoirs
                    modeled = results[m][f'outflow_{node}']
                except:
                    ### for mainstem nodes
                    modeled = results[m][f'link_{node}']
            elif m in base_models:
    ...
    if use2nd:
        plt.savefig(f'{fig_dir}streamflow_3plots_{models[0]}_{models[1]}.png', bbox_inches='tight')
    else:
        plt.savefig(f'{fig_dir}streamflow_3plots_{models[0]}.png', bbox_inches='tight')
    plt.show()


### nhm only - slides 36-39 in 10/24/2022 presentation
plot_3part_flows(results, ['nhmv10'], 'pepacton')

### nhm vs nwm - slides 40-42 in 10/24/2022 presentation
plot_3part_flows(results, ['nhmv10', 'nwmv21'], 'pepacton')

### nhm vs weap (with nhm backup) - slides 60-62 in 10/24/2022 presentation
plot_3part_flows(results, ['nhmv10', 'WEAP_23Aug2022_gridmet'], 'pepacton')

### nhm vs pywr-nhm - slides 60-62 in 10/24/2022 presentation
plot_3part_flows(results, ['nhmv10', 'pywr_nhmv10'], 'pepacton')

## nwm vs pywr-nwm
plot_3part_flows(results, ['nwmv21', 'pywr_nwmv21'], 'pepacton')

## pywr-nwm vs pywr-nwm_withLakes
plot_3part_flows(results, ['pywr_nwmv21', 'pywr_nwmv21_withLakes'], 'pepacton')
...
def plot_weekly_flow_distributions(results, models, node, colors=['0.5', '#67a9cf', '#ef8a62']):
    ...
    if use2nd:
        plt.savefig(f'{fig_dir}streamflow_weekly_{models[0]}_{models[1]}.png', bbox_inches='tight')
    else:
        plt.savefig(f'{fig_dir}streamflow_weekly_{models[0]}.png', bbox_inches='tight')
    plt.show()


### nhm only - slides 36-39 in 10/24/2022 presentation
plot_weekly_flow_distributions(results, ['nhmv10'], 'pepacton')

### nhm vs nwm - slides 35-37 in 10/24/2022 presentation
plot_weekly_flow_distributions(results, ['nhmv10', 'nwmv21'], 'pepacton')

### nhm vs weap (with nhm backup) - slides 68 in 10/24/2022 presentation
plot_weekly_flow_distributions(results, ['nhmv10', 'WEAP_23Aug2022_gridmet'], 'pepacton')

### nhm vs pywr-nhm - slides 68 in 10/24/2022 presentation
plot_weekly_flow_distributions(results, ['nhmv10', 'pywr_nhmv10'], 'pepacton')

## nwm vs pywr-nwm
plot_weekly_flow_distributions(results, ['nwmv21', 'pywr_nwmv21'], 'pepacton')

## pywr-nwm vs pywr-nwm_withLakes
plot_weekly_flow_distributions(results, ['pywr_nwmv21', 'pywr_nwmv21_withLakes'], 'pepacton')
    ...
    for j, node in enumerate(nodes):
        ...
        for i, m in enumerate(models):
            ...
            if np.sum(modeled) > 0:
                ...
                #     exceedance = np.arange(1., len(df_obs) + 1.) / len(df_obs)
                #     logmax = df_obs.max()
                #     logmin = df_obs.min()
                ...
                #         dx = exceedance[k + 1] - exceedance[k]
...
results_metrics = get_error_metrics(results, radial_models, nodes)
...
def plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = True, useweap = True, usepywr = True):
    ...
    reslabeldict = {'pepacton': 'Pep', 'cannonsville': 'Can', 'neversink': 'Nev', 'prompton': 'Pro',
    ...
    for k, metric in enumerate(metrics):
        ...
        for j, node in enumerate(nodes):
            ...
            ax.text(
                np.mean(x1), ax.get_ylim()[1] + 0.01 * (ax.get_ylim()[1] - ax.get_ylim()[0]), reslabeldict[node],
    ...
    plt.savefig(f'{fig_dir}/radialMetrics_{filename_res}_{filename_mod}.png', bbox_inches='tight', dpi=300)
    plt.show()


### nhm vs nwm only, pepacton only - slides 48-54 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = False, useweap = False, usepywr = False)

### nhm vs nwm only, all reservoirs - slides 55-58 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = True, useweap = False, usepywr = False)

### nhm vs nwm vs weap only, pepaction only - slides 69 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = False, useweap = True, usepywr = False)

### nhm vs nwm vs weap only, all reservoirs - slides 70 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = True, useweap = True, usepywr = False)

### all models, pepaction only - slides 72-73 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = False, useweap = True, usepywr = True)

### all models, all reservoirs - slides 74-75 in 10/24/2022 presentation
plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep = True, useweap = True, usepywr = True)
