...
site_matches_link = [
    [
        "delLordville",
        ["01427207"],
        ["cannonsville", "pepacton", "01425000", "01417000"],
    ],
    ...
    [
        ...
        ["01463620"],
        ["assunpink", "01463620"],
    ],  ## note, should get downstream junction, just using reservoir-adjacent gage for now
    [
        "outletSchuylkill",
        ["01474500"],
        ["ontelaunee", "stillCreek", "blueMarsh", "greenLane", "01470960"],
    ],
]
...
def plot_interactive_streamflow_stack(
    node,
    model,
    group_flow=True,
    ...
    base_flow = get_base_results(
        "../input_data/", "obs", release_data.index, results_set="major_flow"
    )
    ...
    non_nyc_reservoirs = [
        i for i in contributing if (i not in nyc_reservoirs) and (i in reservoir_list)
    ]
    unmanaged_flows = [i for i in contributing if (i not in reservoir_list)]

    flow_groups = {
        "NYC Reservoir Releases": nyc_reservoirs,
    ...
    use_inflows = [
        i for i in contributing if (i not in use_releases) and (i in contributing)
    ]
    ...
    contributions = pd.concat(
        [release_contributions, inflow_contributions, node_inflow], axis=1
    )
    ...
    if group_flow:
        ...
        contributions.loc[:, "Unmanaged Flows Upstream"] = (
            contributions.loc[:, "Unmanaged Flows Upstream"]
            + contributions.loc[:, node]
        )
    ...
    total_flow_plot = (
        alt.Chart(
            total_flows_stacked,
            title="Delaware River Streamflow at Trenton, PA",
            width=plot_width,
            height=upper_plot_height,
        )
        ...
        .encode(
            ...
            y=alt.Y(
                "Flow:Q",
                title="Flow (MGD)",
                scale=alt.Scale(type="log", domain=[1000, 100000]),
            ),
            color=alt.Color(
                "Data Source:N",
                scale=alt.Scale(
                    domain=[
                        ...
                        "Other Reservoir Releases",
                        "Unmanaged Flows Upstream",
                    ],
                    range=[
                        obs_flow_color,
                        sim_flow_color,
                        "black",
                        nyc_reservoir_color,
                        other_reservoir_color,
                        unmanaged_flow_color,
                    ],
                ),
                legend=alt.Legend(values=total_flows.columns.to_list()),
            ),
        )
        .add_selection(zoom)
    )

    total_flow_plot.configure_title(fontSize=14, subtitlePadding=25)
    # target_line = alt.Chart(pd.DataFrame({'Flow Target': [target]})).mark_rule().encode(y='Flow Target')

    contribution_plot = (
        alt.Chart(sim_percentages, width=plot_width, height=lower_plot_height)
        .mark_area(strokeWidth=0)
        .encode(
            alt.X(
                "Date:T",
            ...
            alt.Y(
                "sum(Flow):Q",
                title="Percentage of Total Observed Flow",
    ...
    plot = (
        alt.vconcat(total_flow_plot, (contribution_plot + ideal_line))
