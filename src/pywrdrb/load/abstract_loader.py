    ...
    def get_base_results(
        ...
        if results_set == "all":
            ...
            for realization_id in realization_ids:
                ...
                keep_cols = [c for c in gage_flow.columns if c in majorflow_set]
                data[realization_id] = gage_flow[keep_cols]
        ...
        elif results_set == "flood_gage_flow":
            # Filter to only include flood monitoring nodes
            for realization_id in realization_ids:
                gage_flow = data[realization_id]
                # Keep only flood monitoring node columns that exist
                available_flood_nodes = [n for n in flood_monitoring_nodes if n in gage_flow.columns]
                if len(available_flood_nodes) == 0:
                    print(f"Warning: No flood monitoring node data found in gage_flow_mgd.csv")
                    data[realization_id] = pd.DataFrame()
                else:
                    gage_flow = gage_flow.loc[:, available_flood_nodes]
                    data[realization_id] = gage_flow
