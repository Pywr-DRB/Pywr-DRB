...
import numpy as np
import pandas as pd
from pywr.parameters import Parameter, IndexParameter, load_parameter
...
class LowerBasinDroughtLevel(IndexParameter):
    """
    ...
    ------------------------
    int
        Declared drought stage (hysteresis-corrected):

        * **0 — Normal**: Beltzville ≥ 73.7 % AND Blue Marsh ≥ 68.9 % of DRBC usable storage,
          sustained for 30 consecutive days (or immediately after a spill event).
        * **1 — LB Drought Warning**: Either reservoir below its normal-stage threshold
          (entry is instantaneous; exit requires 30 consecutive days above both warning thresholds
          or a spill event).
        * **2 — LB Drought**: BOTH Beltzville < 38.0 % AND Blue Marsh < 36.8 % of DRBC usable
          storage for **three consecutive simulation days** (entry); exit requires 30 consecutive
          days where the drought condition is no longer met, or a spill event at either reservoir.

    Note: inherits from ``IndexParameter`` so Pywr exposes ``value()`` = ``float(index())``.
    Use as ``index_parameter`` in ``IndexedArrayParameter`` (e.g. ``lb_drought_factor_delivery_nj``).
    ...
    **Hysteresis design** (Water Code §2.5.6.E):

    * *Entry* is fast — the declared stage can increase immediately when conditions worsen.
    * *Exit* requires 30 consecutive days where the instantaneous stage is lower than the
      declared stage, OR a spill event at either reservoir (storage ≥ 100 % of DRBC usable
      capacity).  On a spill, the declared stage drops to Normal (0) immediately.

    Three per-scenario counters are maintained:

    * ``drought_days_below`` — consecutive end-of-timestep days where BOTH reservoirs are below
      the LB Drought (lower) thresholds; used to trigger the stage-2 persistence requirement.
    * ``recovery_days`` — consecutive end-of-timestep days where the instantaneous stage is
      lower than the declared stage; triggers exit when it reaches 30.
    * ``declared_stage`` — the officially declared LB drought stage (0/1/2) after hysteresis.
    ...
    Policy Grounding
    ----------------
    Each provision below shows the verbatim Water Code text, how it maps to this
    class, and any modeling considerations or known approximations.

    **Stage 0 — Normal (§2.5.6.B)**

        *What it says:* "lower basin conditions are 'normal' (as defined by Beltzville
        Reservoir storage above Elevation 615 m.s.l. and Blue Marsh Reservoir storage
        above Elevation 283 m.s.l.)"

        *Model interpretation:* Stage 0 is returned when BOTH reservoirs are above their
        respective warning fractions (``BETZ_WARNING_FRAC = 0.737``,
        ``BM_WARNING_FRAC = 0.689``) AND the declared stage has been held at 0 (or
        reached 0 via the 30-day exit).

        *Considerations:* The Water Code defines "normal" as a conjunctive (AND) condition
        — both must be above threshold simultaneously.  The model's exit logic (both above
        warning fracs for 30 consecutive days) faithfully reproduces this AND requirement.

    **Stage 1 — LB Drought Warning (§2.5.6.C, entry)**

        *What it says:* "When the storage in Beltzville Reservoir falls below elevation
        615 m.s.l. (73.7% of storage capacity) and the storage in Blue Marsh Reservoir
        falls below elevation 283 m.s.l. (68.9% of storage capacity) the Delaware River
        Basin Commission shall declare a 'drought warning' condition in the lower basin"
        (Resolution No. 88-22 Revised)

        *Model interpretation:* ``value()`` returns 1 when ``betz_frac < 0.737 OR
        bm_frac < 0.689``.  The model uses a **disjunctive** (OR) trigger — warning fires
        when *either* reservoir is below its threshold.

        *Considerations:* The Water Code uses AND for the formal Commission declaration.
        The OR interpretation is a deliberate modeling choice: ``priority_use_during_drought``
        (Table 1 in §2.5.5) assigns both Beltzville and Blue Marsh to Priority 1,
        meaning either reservoir should begin contributing to Trenton flow objectives once
        it individually falls below its 73.7%/68.9% threshold.  Using OR aligns with the
        priority table operational logic and provides a more conservative (earlier) warning
        signal.  Affects D4 attribution: NJ diversion caps should in principle also reduce
        to 70 mgd (Warning) under this stage — see NJ gap note below.

    **Stage 1 — LB Drought Warning (§2.5.6.C.7, exit)**

        *What it says:* "When the storage levels in Beltzville and Blue Marsh Reservoirs
        simultaneously exceed their respective lower basin 'drought warning' storage levels
        for 30 consecutive days, or when either of those reservoirs spill, the lower basin
        'drought warning' operation shall automatically terminate, and 'normal' operations
        shall be resumed, unless the Commission unanimously agrees otherwise."

        *Model interpretation:* ``recovery_days`` increments each end-of-timestep day where
        the instantaneous stage is below the declared stage (i.e., both above warning
        fracs → ``inst = 0 < declared = 1``).  After 30 such days, ``declared_stage`` drops
        to 0.  Spill (``betz_frac >= 1.0 OR bm_frac >= 1.0``) resets ``declared_stage``
        to 0 immediately in ``after()``.

        *Considerations:* "Simultaneously exceed" means both must be above thresholds on
        every one of the 30 days — a single day below resets the counter (because
        ``inst`` rises back to 1, which triggers the ``inst >= declared`` branch and
        resets ``recovery_days``).  This is faithfully reproduced by the model's counter
        logic.  Spill exit is implemented as storage reaching 100 % of DRBC usable
        conservation capacity (``betz_frac >= 1.0`` or ``bm_frac >= 1.0``), not the
        reservoir's total physical capacity — a slight approximation since DRBC usable
        storage is already the policy-relevant ceiling.

    **Stage 2 — LB Drought (§2.5.6.D / §2.5.6.E, entry)**

        *What it says:* "When storage in Beltzville Reservoir falls below elevation
        590 m.s.l. (38.0% of storage capacity) and storage in Blue Marsh Reservoir falls
        below elevation 273 m.s.l. (36.8% of capacity) and remains below such levels for
        three consecutive days, the Delaware River Basin Commission shall, pursuant to
        Article 10.4 of the Compact, declare a lower basin 'drought' emergency condition"
        (Resolution No. 88-22 Revised)

        *Model interpretation:* ``drought_days_below`` counts consecutive end-of-timestep
        days where BOTH are below ``BETZ_DROUGHT_FRAC = 0.380`` AND
        ``BM_DROUGHT_FRAC = 0.368``.  Stage 2 fires in ``value()`` when today is also
        below both thresholds AND ``drought_days_below >= DROUGHT_PERSIST_DAYS - 1 = 2``
        (i.e., today is day 3).  The AND/3-day persistence is implemented exactly as
        written.

        *Considerations:* The 3-day persistence requirement is correctly implemented.
        Stage 2 requires a formal Commission declaration in practice; the model treats
        the physical threshold breach as a deterministic proxy.  In ensemble runs, the
        3-day persistence prevents spurious drought declarations from single-day dips
        near the threshold.

    **Stage 2 — LB Drought (exit)**

        *What it says:* The Water Code does not include a symmetric explicit exit clause
        for stage 2 in the same form as §2.5.6.C.7.  Operation terminates "until
        termination of the drought emergency declaration by the Commission" (§2.5.5).
        Conservation releases "will terminate and return to normal at the same time as
        augmented conservation releases are restored at the New York City Delaware
        reservoirs" (§2.5.5).

        *Model interpretation:* The same ``recovery_days`` / 30-day hysteresis logic
        applied to stage 1 is also applied to stage 2.  When both reservoirs rise above
        their drought thresholds (38.0%/36.8%), ``inst`` drops from 2 to 1 or 0;
        ``recovery_days`` starts counting; after 30 consecutive days, ``declared_stage``
        drops to the current ``inst`` value (which may be 1 if still in warning territory,
        or 0 if both are already above warning thresholds).

        *Considerations:* This is a deliberate approximation.  The Water Code ties drought
        exit to a unanimous Commission declaration, not to an automatic storage threshold.
        The 30-day rule is the closest quantitative proxy available without modeling the
        Commission decision process.  Note the cascade: drought (2) → warning (1) requires
        30 recovery days from the drought condition, then warning (1) → normal (0) requires
        another 30 days where both are above warning thresholds.  This two-step exit is
        more conservative than a direct 2 → 0 jump and reflects the institutional reality
        of multi-step declaration rescission.

    **Conservation releases (Water Code §2.5.5 Table 4)**
        ...
        *What it says:*
        ...
        .. code-block:: text
            ...
            * With Western Berks Water Authority release included.

        "conservation releases from the listed lower basin reservoirs shall be modified
        as set forth in Table 4 of Section 2.5.5 commencing with the declaration of a
        lower basin 'drought warning' condition" (§2.5.6.C.4)

        *Model interpretation:* ``conservation_releases`` uses 50 cfs (Blue Marsh with
        WBWA) and 35 cfs (Beltzville) for stage 0.  ``lower_basin_drought_conservation_releases``
        uses 30 cfs (Blue Marsh with WBWA) and 15 cfs (Beltzville) for stages 1–2.
        ``LowerBasinMaxMRFContribution.value()`` switches between the two dicts based on
        ``drought_level_agg_lb >= 1``.

        *Considerations:* Both the normal and drought values use the WBWA-inclusive
        figures (50/30 cfs Blue Marsh) rather than the table-base values (41/21 cfs).
        The model applies the same reduced releases for both stage 1 (Drought Warning) and
        stage 2 (Drought), consistent with "commencing with the declaration of a lower basin
        'drought warning' condition."  Nockamixon conservation release switching is also
        implemented (11 → 7 cfs) though Nockamixon is only activated under drought
        conditions in ``LowerBasinMaxMRFContribution``.

    **§2.5.6.C.1 / §2.5.6.D.1 — NJ diversion caps under LB drought (implemented 2026-05-28)**

        *What it says:*
        §2.5.6.C.1: "The total diversion by the State of New Jersey during lower basin
        'drought warning' conditions ... shall not exceed a running average of 70 million
        gallons per day (mgd)"
        §2.5.6.D.1 / §2.5.6.E.1: "The total diversion by New Jersey during lower basin
        drought conditions shall not exceed a running average of 65 mgd"
        Compare to §2.5.6.B.1 (Normal): NJ total diversion ≤ 100 mgd monthly average.

        *Model interpretation:* ``lb_drought_factor_delivery_nj`` is an
        ``IndexedArrayParameter`` indexed on ``drought_level_agg_lb`` with factors
        1.0 / 0.70 / 0.65 for stages 0 / 1 / 2 (applied to the 100 MGD baseline).
        ``combined_drought_factor_delivery_nj`` = min(``drought_factor_delivery_nj``,
        ``lb_drought_factor_delivery_nj``) — ensures the more restrictive of the NYC
        drought regime and the LB drought regime always governs NJ delivery.
        ``FfmpNjRunningAvg`` in ``model_builder.py`` is wired to this combined factor.

        *Considerations:* The min() combination is correct because the Water Code caps
        are absolute limits: the LB Warning cap (70 MGD) overrides the NYC level-4 cap
        (90 MGD) whenever LB drought Warning is declared simultaneously with NYC level 4.
        The ``FfmpNjRunningAvg`` running-average reset logic is also correctly triggered
        by changes in the combined factor — a change in *either* the NYC stage *or* the
        LB stage resets the delivery budget to the new level.
    ...
    References
    ----------
    DRBC Water Code §2.5.6 — Lower Basin Drought Stages (Resolution No. 88-22 Revised,
        as Amended by Resolution No. 2002-33)
    DRBC Water Code §2.5.5 Table 4 — Conservation Releases
    DRBC Water Code §2.5.6.C.7 — Ending Lower Basin Drought Warning (30-day exit / spill)
    DRBC Water Code §2.5.6.D — LB Drought declaration (3-day persistence, both below)
    """

    # -------------------------------------------------------------------------
    # Thresholds — verbatim from Water Code §2.5.6 (fraction of DRBC usable storage)
    # -------------------------------------------------------------------------

    # Normal / Drought-Warning boundary (§2.5.6.B / §2.5.6.C)
    # Water Code: "Beltzville Reservoir storage above Elevation 615 m.s.l." (73.7%)
    #             "Blue Marsh Reservoir storage above Elevation 283 m.s.l." (68.9%)
    BETZ_WARNING_FRAC = 0.737   # Beltzville Elev 615 m.s.l. → 73.7% of 13,500 MG
    BM_WARNING_FRAC   = 0.689   # Blue Marsh  Elev 283 m.s.l. → 68.9% of  7,450 MG

    # Warning / Drought boundary (§2.5.6.D / §2.5.6.E)
    # Water Code: "storage in Beltzville Reservoir falls below elevation 590 m.s.l.
    #              (38.0% of storage capacity) and storage in Blue Marsh Reservoir
    #              falls below elevation 273 m.s.l. (36.8% of capacity)"
    BETZ_DROUGHT_FRAC = 0.380   # Beltzville Elev 590 m.s.l. → 38.0% of 13,500 MG
    BM_DROUGHT_FRAC   = 0.368   # Blue Marsh  Elev 273 m.s.l. → 36.8% of  7,450 MG

    # DRBC usable conservation-pool storage (MG)
    # Source: DRBC dashboard https://drbc.maps.arcgis.com/apps/dashboards/690464a...
    MAX_VOL_BELTZVILLE = 13_500   # MG
    MAX_VOL_BLUEMARSH  =  7_450   # MG

    # §2.5.6.D: "remains below such levels for three consecutive days" before
    # the Commission declares a lower basin "drought" emergency condition.
    DROUGHT_PERSIST_DAYS = 3

    # §2.5.6.C.7: "simultaneously exceed their respective lower basin 'drought
    # warning' storage levels for 30 consecutive days, or when either of those
    # reservoirs spill" → automatic termination of drought warning / drought stage.
    RECOVERY_PERSIST_DAYS = 30

    def __init__(self, model, node_beltzville, node_blueMarsh,
                 betz_warning_frac=None, bm_warning_frac=None,
                 recovery_persist_days=None, **kwargs):
        """
        ...
        betz_warning_frac : float, optional
            Override for BETZ_WARNING_FRAC (default 0.737 — §2.5.6.C).
            Used by Sobol/sensitivity sweeps to sample around the class-level default.
            Setting this as an instance attribute shadows the class constant at runtime.
        bm_warning_frac : float, optional
            Override for BM_WARNING_FRAC (default 0.689 — §2.5.6.C).
            Same injection mechanism as betz_warning_frac.
        recovery_persist_days : int, optional
            Override for RECOVERY_PERSIST_DAYS (default 30 — §2.5.6.C.7).
            Same injection mechanism; cast to int so the loop counter comparison works.
        **kwargs
            Additional keyword arguments passed to the base Parameter class.
        """
        super().__init__(model, **kwargs)
        ...
        # Instance-level threshold overrides — shadow class constants when set.
        # Python attribute lookup finds instance attrs before class attrs, so
        # value() and after() automatically use these without any other changes.
        if betz_warning_frac is not None:
            self.BETZ_WARNING_FRAC = float(betz_warning_frac)
        if bm_warning_frac is not None:
            self.BM_WARNING_FRAC = float(bm_warning_frac)
        if recovery_persist_days is not None:
            self.RECOVERY_PERSIST_DAYS = int(recovery_persist_days)

    def setup(self):
        """Allocate per-scenario arrays for drought persistence and recovery tracking."""
        super().setup()
        num_scenarios = len(self.model.scenarios.combinations)
        # Counts end-of-timestep days (completed before the *next* value() call) where
        # BOTH reservoirs were below the LB Drought (lower) thresholds.
        self.drought_days_below = np.zeros(num_scenarios, dtype=np.int32)
        # Counts consecutive days where the instantaneous stage is below the declared stage
        # (i.e., days into a potential exit from the current drought level).  Resets to 0
        # whenever conditions worsen (inst >= declared) or exit threshold is reached.
        self.recovery_days = np.zeros(num_scenarios, dtype=np.int32)
        # Officially declared stage after applying hysteresis (§2.5.6.E).  This is what
        # index() returns.  Entry is immediate; exit requires RECOVERY_PERSIST_DAYS or spill.
        self.declared_stage = np.zeros(num_scenarios, dtype=np.int32)

    def reset(self):
        """Reset all per-scenario counters to zero at simulation start."""
        self.drought_days_below[:] = 0
        self.recovery_days[:] = 0
        self.declared_stage[:] = 0

    def index(self, timestep, scenario_index):
        """
        Return the declared (hysteresis-corrected) LB drought stage for a single scenario.

        This is an ``IndexParameter`` — Pywr calls ``index()`` (returning int) and
        exposes ``value()`` automatically as ``float(self.index(...))``.  Returning int
        is required so that ``IndexedArrayParameter`` can use this as its index_parameter
        (Pywr asserts ``isinstance(index_parameter, IndexParameter)`` on load).
        ...
        -------
        int
            Declared stage: 0 (Normal), 1 (LB Drought Warning), or 2 (LB Drought).

        Notes
        -----
        Entry is immediate — the declared stage jumps up as soon as the instantaneous
        conditions warrant it (same-day response).  Exit is slow — the declared stage
        only drops after :attr:`RECOVERY_PERSIST_DAYS` consecutive days where the
        instantaneous stage is lower, or immediately upon a spill event (handled in
        ``after()``).

        The drought-days-below counter (``drought_days_below``) is updated in ``after()``
        using end-of-timestep volumes; here it reflects the count accumulated through the
        *previous* timestep, so the 3-day persistence check is:
        ``today_drought AND drought_days_below >= DROUGHT_PERSIST_DAYS - 1``.
        """
        sid = scenario_index.global_id
        ...
        # --- Instantaneous stage (what current conditions warrant right now) ---
        ...
        if today_drought and self.drought_days_below[sid] >= (self.DROUGHT_PERSIST_DAYS - 1):
            inst = 2
        elif (betz_frac < self.BETZ_WARNING_FRAC) or (bm_frac < self.BM_WARNING_FRAC):
            inst = 1
        else:
            inst = 0

        # --- Apply hysteresis (§2.5.6.E) ---
        # Entry is fast: if instantaneous stage exceeds the declared stage, update immediately.
        # Exit is slow: if conditions have improved, hold the declared stage until after()
        # has counted 30 consecutive recovery days (or a spill has been detected).
        if inst >= self.declared_stage[sid]:
            return int(inst)
        else:
            return int(self.declared_stage[sid])

    def after(self):
        """
        Update all per-scenario counters and the declared drought stage at end of timestep.

        This method runs after routing (Pywr calls it once per timestep after ``value()``).
        It reads end-of-timestep storage volumes and performs three updates in order:

        1. **drought_days_below** — increments if both reservoirs are below the LB Drought
           thresholds (38.0 % / 36.8 %); resets to 0 otherwise.
        2. **Instantaneous stage** — recomputes using the *updated* counter (so after 3
           consecutive days the inst stage becomes 2 here, and ``declared_stage`` is set
           to 2 ready for the next ``value()`` call).
        3. **declared_stage / recovery_days** — applies hysteresis per §2.5.6.E:

           * Spill at either reservoir (storage ≥ 100 % DRBC usable): immediate reset to
             Normal (0), recovery_days = 0.
           * inst ≥ declared: entry or steady-state — set declared = inst, reset
             recovery_days.
           * inst < declared: potential exit — increment recovery_days; if
             recovery_days ≥ :attr:`RECOVERY_PERSIST_DAYS`, accept exit (declared = inst,
             recovery_days = 0).
        """
        betz_vols = np.asarray(self.node_beltzville.volume).ravel()
        bm_vols   = np.asarray(self.node_blueMarsh.volume).ravel()
        n = len(self.declared_stage)
        ...
        for s in range(n):
            ...
            # ------------------------------------------------------------------ #
            # 1. Spill condition — either reservoir at or above conservation-pool #
            #    capacity (§2.5.6.E: spill is an immediate exit trigger).         #
            # ------------------------------------------------------------------ #
            spill = (betz_frac >= 1.0) or (bm_frac >= 1.0)

            # ------------------------------------------------------------------ #
            # 2. Update drought_days_below counter.                               #
            # ------------------------------------------------------------------ #
            today_drought = (betz_frac < self.BETZ_DROUGHT_FRAC) and \
                            (bm_frac   < self.BM_DROUGHT_FRAC)
            if today_drought:
            ...
            # ------------------------------------------------------------------ #
            # 3. Compute instantaneous stage using the UPDATED counter.           #
            #    drought_days_below now includes today, so stage-2 requires >= 3. #
            # ------------------------------------------------------------------ #
            if today_drought and \
                    self.drought_days_below[s] >= self.DROUGHT_PERSIST_DAYS:
                inst = 2
            elif (betz_frac < self.BETZ_WARNING_FRAC) or \
                    (bm_frac < self.BM_WARNING_FRAC):
                inst = 1
            else:
                inst = 0

            # ------------------------------------------------------------------ #
            # 4. Update declared_stage with hysteresis (§2.5.6.E).               #
            # ------------------------------------------------------------------ #
            if spill:
                # Spill → immediate full exit from any drought stage
                self.declared_stage[s] = 0
                self.recovery_days[s]  = 0
            elif inst >= self.declared_stage[s]:
                # Entry or steady-state: accept immediately, reset recovery counter
                self.declared_stage[s] = inst
                self.recovery_days[s]  = 0
            else:
                # inst < declared: potential exit — count recovery days
                self.recovery_days[s] += 1
                if self.recovery_days[s] >= self.RECOVERY_PERSIST_DAYS:
                    # 30-day threshold met: officially exit to current inst stage
                    self.declared_stage[s] = inst
                    self.recovery_days[s]  = 0
    ...
    def load(cls, model, data):
        ...
        model : pywr.Model
            The Pywr model instance.
        data : dict
            Dictionary from JSON/YAML model input.  Standard Pywr parameter fields
            (``type``, ``name``, etc.) are passed through to the base class via ``**data``.
            Optional sensitivity-override keys also accepted and forwarded to ``__init__``:

            * ``betz_warning_frac`` (float) — overrides BETZ_WARNING_FRAC (0.737)
            * ``bm_warning_frac`` (float) — overrides BM_WARNING_FRAC (0.689)
            * ``recovery_persist_days`` (int) — overrides RECOVERY_PERSIST_DAYS (30)
