...
if __name__ == "__main__":
    ...
    for climate in ["1960s", "aorc", "2050s"]:
        c = {"climate": climate, "calibration": calibration, "landcover": landcover}
    ...
    for config in rank_configs:
        print(
            f'Preparing data for {config["climate"]} {config["calibration"]} {config["landcover"]}'
        )
        ...
        dataset_label = (
            f'wrf{config["climate"]}_{config["calibration"]}_{config["landcover"]}'
        )
        ...
        predict_inflows_diversions(
            dataset_label, start_date_training, end_date_training, make_figs=True
        )
