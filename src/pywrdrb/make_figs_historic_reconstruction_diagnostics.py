...
if __name__ == "__main__":
    ...
    for model in pywr_models:
        ...
        major_flows[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, results_set="major_flow", datetime_index=datetime_index
        )
        storages[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, results_set="res_storage", datetime_index=datetime_index
        )
        reservoir_releases[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, results_set="res_release", datetime_index=datetime_index
        )
        ...
        inflows[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "inflow", datetime_index=datetime_index
        )
        nyc_release_components[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "nyc_release_components", datetime_index=datetime_index
        )
        ibt_demands[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "ibt_demands", datetime_index=datetime_index
        )
        ibt_diversions[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "ibt_diversions", datetime_index=datetime_index
        )
        catchment_consumptions[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "catchment_consumption", datetime_index=datetime_index
        )
        mrf_targets[f"pywr_{model}"], datetime_index = get_pywr_results(
            output_dir, model, "mrf_targets", datetime_index=datetime_index
        )
    ...
    for model in base_models:
        ...
        major_flows[model], datetime_index = get_base_results(
            input_dir, model, results_set="major_flow", datetime_index=datetime_index
        )
        ...
        for model in pywr_models:
            for node in ["delMontague", "delTrenton"]:
                plot_NYC_release_components_combined(
                    ...
                    use_log=True,
                    start_date=start_date_short_preobs,
                    end_date=end_date_short_preobs,
                    fig_dir=fig_dir,
                )
