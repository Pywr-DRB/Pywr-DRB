...
class Options:
    ...
    nyc_nj_demand_source : str = "historical"
...
class ModelBuilder:
    ...
    def make_model(self):
        ...
        self.add_parameter_erq_releases()
    ...
    def add_parameter_nyc_and_nj_demands(self):
        ...
        # If 'historical', use internal historical extrapolated deliveries based on USGS flows
        if nyc_nj_demand_source == "historical":
            # NYC
        ...
        # If 'custom', then take from the custom inflow_type folder
        # These are generated by pywrdrb.pre.ExtrapolatedDiversionPreprocessor
        elif nyc_nj_demand_source == "custom":
            inflow_type = self.inflow_type

            # Check if using ensemble diversions or single-scenario diversions
            if inflow_ensemble_indices is not None:
                # Using ensemble diversions - use DiversionEnsemble parameter
                # NYC
                model_dict["parameters"][f"demand_nyc"] = {
                    "type": "DiversionEnsemble",
                    "diversion_location": "nyc",
                    "inflow_type": inflow_type,
                    "diversion_ensemble_indices": inflow_ensemble_indices,
                }

                # NJ
                model_dict["parameters"][f"demand_nj"] = {
                    "type": "DiversionEnsemble",
                    "diversion_location": "nj",
                    "inflow_type": inflow_type,
                    "diversion_ensemble_indices": inflow_ensemble_indices,
                }
            else:
    ...
    def add_parameter_nyc_reservoirs_operational_regimes(self):
        ...
        ### Indexed arrays that dictate cutbacks to NYC & NJ deliveries, based on current NYC storage level
        for demand in demands:
        ...
        ### LB drought delivery factors for NJ (§2.5.6.C.1 and §2.5.6.D.1)
        ### Normal: ≤100 MGD (factor 1.0); Warning: ≤70 MGD (factor 0.70); Drought: ≤65 MGD (factor 0.65)
        ### Source: Water Code §2.5.6.C.1 (70 MGD cap), §2.5.6.D.1 (65 MGD cap)
        for lb_level in [0, 1, 2]:
            model_dict["parameters"][f"lb_level{lb_level}_factor_delivery_nj"] = {
                "type": "constant",
                "url": pn.operational_constants.get_str("constants.csv"),
                "column": "value",
                "index_col": "parameter",
                "index": f"lb_level{lb_level}_factor_delivery_nj",
            }

        ### IndexedArray selecting the LB drought factor based on current LB drought stage (0/1/2)
        model_dict["parameters"]["lb_drought_factor_delivery_nj"] = {
            "type": "indexedarray",
            "index_parameter": "drought_level_agg_lb",
            "params": [f"lb_level{lb_level}_factor_delivery_nj" for lb_level in [0, 1, 2]],
        }

        ### Combined NJ drought factor = min(NYC-driven factor, LB-driven factor)
        ### Ensures the more restrictive of the two drought regimes always governs NJ delivery.
        model_dict["parameters"]["combined_drought_factor_delivery_nj"] = {
            "type": "aggregated",
            "agg_func": "min",
            "parameters": [
    ...
    def add_parameter_nyc_and_nj_delivery_constraints(self):
        ...
        # Uses combined_drought_factor_delivery_nj = min(NYC-driven factor, LB-driven factor).
        # LB contribution: §2.5.6.C.1 (≤70 MGD during LB Drought Warning) and
        #                  §2.5.6.D.1 (≤65 MGD during LB Drought).
        model_dict["parameters"]["max_flow_ffmp_delivery_nj"] = {
            ...
            "drought_factor": "combined_drought_factor_delivery_nj",
        }
    ...
    def add_parameter_erq_releases(self):
        """
        Add parameters implementing the Excess Release Quantity (ERQ) from NYC reservoirs.

        Legal Basis
        -----------
        1954 U.S. Supreme Court Decree, Art. III-B-1(c)-(d):

        Art. III-B-1(c): NYC must release 83% of the amount by which its annual
        consumption falls short of safe yield (≥1,665 MGD after Cannonsville).

        Art. III-B-1(d): The ERQ is released June 15 – March 15 at a rate designed
        to deliver the full quantity in 120 days; the annual total shall not exceed
        70 billion gallons.

        Design
        ------
        `erq_release_nyc` (ERQRelease) computes the daily aggregate target across all
        3 NYC reservoirs. It is distributed equally (1/3 per reservoir) and added to
        each reservoir's existing `downstream_release_target_{reservoir}` parameter,
        which already includes FFMP-mandated releases, flood control, and
        Montague/Trenton MRF contributions.

        The equal 1/3 distribution is a deliberate simplification: the Decree states
        releases shall come "from all its storage reservoirs" without specifying a
        split. A proportional-storage allocation would be more realistic but requires
        a custom balancing parameter; this can be refined in a future revision.

        The `erq_fraction` (default 0.83) is designed as a Sobol sensitivity
        parameter — varying it characterizes how the cooperative reciprocity
        obligation affects each decree party's RRV metrics.

        Sensitivity parameter injection
        --------------------------------
        `erq_fraction` can be overridden at model-build time by patching:
            model_dict["parameters"]["erq_release_nyc"]["erq_fraction"] = <value>
        This is handled by run_model.run_single() when "erq_fraction" appears in
        sensitivity_params.
        """
        model_dict = self.model_dict

        # Aggregate ERQ parameter: total daily release across all 3 NYC reservoirs
        model_dict["parameters"]["erq_release_nyc"] = {
            "type": "ERQRelease",
            "erq_fraction": 0.83,          # Decree Art. III-B-1(c): "83 per cent"
            "nyc_delivery_node_name": "delivery_nyc",
        }

        # Distribute ERQ equally across Cannonsville, Pepacton, Neversink
        # Decree Art. III-B-1(c): "from all its storage reservoirs in the upper
        # Delaware watershed" — equal split is the simplest defensible allocation.
        model_dict["parameters"]["one_third_erq_multiplier"] = {
            "type": "constant",
            "value": 1.0 / 3.0,
        }

        for reservoir in reservoir_list_nyc:
            model_dict["parameters"][f"erq_release_{reservoir}"] = {
                "type": "aggregated",
                "agg_func": "product",
                "parameters": ["erq_release_nyc", "one_third_erq_multiplier"],
            }
            # Append ERQ release to the existing downstream release target sum.
            # downstream_release_target_{reservoir} already aggregates:
            #   mrf_target_individual + flood_release + mrf_montagueTrenton
            # Adding ERQ here makes it a fourth additive term.
            model_dict["parameters"][f"downstream_release_target_{reservoir}"][
                "parameters"
            ].append(f"erq_release_{reservoir}")
