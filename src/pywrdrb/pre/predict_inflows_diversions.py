...
def get_known_or_predicted_value(
    timeseries, catchment_wc, regressions, node, lag, idx, use_log, mode
):
    """
        ...
        if lag <= 0:
            ...
            if lag == 1:
                ...
                if use_log:
                    eps = 0.001
                    timeseries_t = eps if timeseries_t < eps else timeseries_t
                    timeseries_lagm1_prediction = np.exp(
                        const + slope * np.log(timeseries_t)
                    )
                else:
...
def predict_inflows_diversions(
    dataset_label,
    start_date,
    end_date,
    use_log=True,
    ...
    if ensemble_inflows and catchment_inflows is None:
        ...
        ensemble_input_dir += (
            "synthetic_ensembles/" if "syn" in dataset_label else "historic_ensembles/"
        )

        ensemble_filename = (
            f"{ensemble_input_dir}/catchment_inflow_{dataset_label}.hdf5"
        )
        catchment_inflows = extract_realization_from_hdf5(
            ensemble_filename, realization, stored_by_node=True
        )
        ...
        catchment_inflows = pd.read_csv(
            f"{input_dir}/catchment_inflow_{dataset_label}.csv"
        )
        catchment_inflows.index = pd.DatetimeIndex(catchment_inflows["datetime"])

    catchment_inflows_training = subset_timeseries(
        catchment_inflows, start_date, end_date
    )
    ...
    for node in lag_1_nodes:
        const, slope = regress_future_timeseries(
            catchment_inflows_training,
            node,
            lag,
            use_log=use_log,
    ...
    for pred_lag in [2, 1]:
        node_lags = [
            ("01425000", pred_lag - 2),
            ("01417000", pred_lag - 2),
            ("delLordville", pred_lag - 2),
            ("01436000", pred_lag - 1),
            ("wallenpaupack", pred_lag - 1),
            ("prompton", pred_lag - 1),
            ("shoholaMarsh", pred_lag - 1),
            ("mongaupeCombined", pred_lag),
            ("01433500", pred_lag),
            ("delMontague", pred_lag - 1),
        ]

        for mode in ["regression_disagg", "perfect_foresight", "same_day"]:
            predicted_timeseries[f"{pred_node}_lag{pred_lag}_{mode}"] = np.zeros(
                catchment_inflows.shape[0]
            )
            for node, lag in node_lags:
                predicted_timeseries[f"{pred_node}_lag{pred_lag}_{mode}"] += np.array(
                    [
                        get_known_or_predicted_value(
                            catchment_inflows,
                            catchment_wc,
                            regressions,
                            node,
                        ...
                        for idx in range(catchment_inflows.shape[0])
                    ]
                )
    ...
    for pred_lag in range(4, 0, -1):
        node_lags = [
            ("01425000", pred_lag - 4),
            ("01417000", pred_lag - 4),
            ("delLordville", pred_lag - 4),
            ("01436000", pred_lag - 3),
            ("wallenpaupack", pred_lag - 3),
            ("prompton", pred_lag - 3),
            ("shoholaMarsh", pred_lag - 3),
            ("mongaupeCombined", pred_lag - 2),
            ("01433500", pred_lag - 2),
            ("delMontague", pred_lag - 2),
            ("beltzvilleCombined", pred_lag - 2),
            ("01447800", pred_lag - 2),
            ("fewalter", pred_lag - 2),
            ("01449800", pred_lag - 2),
            ("hopatcong", pred_lag - 1),
            ("merrillCreek", pred_lag - 1),
            ("nockamixon", pred_lag - 1),
            ("delDRCanal", pred_lag),
        ]
    ...
    mode = "regression_agg"
    nonnyc_gage_flow = add_upstream_catchment_inflows(
        catchment_inflows, exclude_NYC=True
    )
    nonnyc_gage_flow_training = subset_timeseries(
        nonnyc_gage_flow, start_date, end_date
    )

    regressions = {}
    for pred_node, pred_lag in zip(
        (
            "delMontague",
            "delMontague",
            "delMontague",
    ...
    for pred_node, pred_lag in zip(
        ...
        predicted_timeseries[f"{pred_node}_lag{pred_lag}_{mode}"] = rollmean_timeseries[
            pred_node
        ]
    ...
    nj_diversions = subset_timeseries(
        nj_diversions, catchment_inflows.index[0], catchment_inflows.index[-1]
    )
        ...
        for mode in ["regression_disagg", "perfect_foresight", "same_day"]:
            predicted_timeseries[f"{pred_node}_lag{pred_lag}_{mode}"] = np.array(
                [
                    ...
                    for idx in range(nj_diversions.shape[0])
                ]
            )
        predicted_timeseries[
            f"{pred_node}_lag{pred_lag}_regression_agg"
        ] = predicted_timeseries[f"{pred_node}_" + f"lag{pred_lag}_regression_disagg"]
    ...
    ### save to csv
    if save_predictions:
        predicted_timeseries.to_csv(
            f"{input_dir}/predicted_inflows_diversions_{dataset_label}.csv", index=False
        )
    ...
    if make_figs:
        ...
        units_conversion = 0.0037854118 if units == "MCM/D" else 1.0
        loc_dict = {
            "delMontague": "Montague",
            "delTrenton": "Trenton",
            "demand_nj": "NJ diversion",
        }
        for mode in [
            "regression_disagg"
        ]:  # , 'regression_agg', 'same_day', 'moving_average', 'perfect_foresight']
            fig, axs = plt.subplots(
                4, 3, figsize=(8, 8), gridspec_kw={"hspace": 0.3, "wspace": 0.3}
            )
            for row, lag in enumerate(range(1, 5)):
                for col, loc in enumerate(["delMontague", "delTrenton", "demand_nj"]):
                    ...
                    if f"{loc}_lag{lag}_{mode}" in predicted_timeseries.columns:
                        ax.scatter(
                            predicted_timeseries[f"{loc}_lag{lag}_perfect_foresight"]
                            * units_conversion,
            ...
            plt.savefig(
                f"{fig_dir}/predict_flows_{mode}_{dataset_label}.png",
                dpi=400,
                bbox_inches="tight",
            )


def predict_ensemble_inflows_diversions(
    dataset_label,
    start_date,
    end_date,
    use_log=True,
    ...
    for i in realizations_per_proc:
        print(
            f"Process {rank}: Making predictions for realization {i+1} of {num_realizations_per_proc}"
        )
    ...
    if rank == 0:
        combined_ensemble_pred = {
            key: val for d in all_ensemble_pred for key, val in d.items()
        }
        output_filename = (
            f"{input_dir}predicted_inflows_diversions_{dataset_label}.hdf5"
        )
