...
def combine_nwmv21_datasets():
    ...
    nwm_gauge_flow = read_modeled_estimates(
        filename,
        sep="\t",
        date_label="UTC_date",
        site_label="site_no",
    ...
    nwm_reach_flow = read_modeled_estimates(
        filename,
        sep,
        date_label,
        site_label="link",
    ...
    nwm_lake_inflow = read_modeled_estimates(
        filename,
        sep,
        date_label,
    ...
    # Export
    all_nwm_flows.to_csv(
        f"{input_dir}modeled_gages/streamflow_daily_nwmv21_mgd.csv", index_label="date"
    )
