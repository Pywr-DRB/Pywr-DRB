...
def download_USGS_data_NYC_NJ_diversions(dates=("1952-01-01", "2022-12-31")):
    ...
    for reservoir in ["cannonsville", "pepacton", "neversink"]:
        hist_flows[reservoir] = hist_flows[[c for c in labels if reservoir in c]].sum(
            axis=1
        )
    hist_flows["NYC_inflow"] = hist_flows[
        ["cannonsville", "pepacton", "neversink"]
    ].sum(axis=1)
    hist_flows = hist_flows[["datetime", "D_R_Canal", "delTrenton", "NYC_inflow"]]
    hist_flows.to_csv(
        input_dir
        + "/usgs_gages/streamflow_daily_usgs_1950_2022_cms_for_NYC_NJ_diversions.csv",
        index=False,
    )
    return hist_flows
...
def extrapolate_NYC_NJ_diversions(loc, make_figs):
    ...
    if loc == "nyc":
        diversion = pd.read_excel(
            f"{input_dir}historic_NYC/Pep_Can_Nev_diversions_daily_2000-2021.xlsx",
            index_col=0,
        )
    ...
    flow = flow.loc[
        np.logical_and(
            flow.index >= diversion.index.min(), flow.index <= diversion.index.max()
        )
    ]
    diversion = diversion.loc[
        np.logical_and(
            diversion.index >= flow.index.min(), diversion.index <= flow.index.max()
        )
    ]
    ...
    if make_figs == True:
        ...
        plt.savefig(
            f"{fig_dir}/extrapolation_{loc}_pt2.png", dpi=400, bbox_inches="tight"
        )
