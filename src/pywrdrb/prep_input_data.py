...
if __name__ == "__main__":
    ...
    df_obs = read_csv_data(
        f"{input_dir}usgs_gages/streamflow_daily_usgs_1950_2022_cms.csv",
        "1945/01/01",
        "2022/12/31",
        units="cms",
        source="USGS",
    ...
    df_nhm = read_csv_data(
        f"{input_dir}modeled_gages/streamflow_daily_nhmv10_mgd.csv",
        start_date,
        end_date,
        units="mgd",
        source="nhm",
    ...
    df_nwm = read_csv_data(
        f"{input_dir}modeled_gages/streamflow_daily_nwmv21_mgd.csv",
        start_date,
        end_date,
        units="mgd",
        source="nwmv21",
    ...
    predict_inflows_diversions(
        "nhmv10", start_date_training, end_date_training, make_figs=True
    )
    predict_inflows_diversions(
        "nwmv21", start_date_training, end_date_training, make_figs=True
    )
    predict_inflows_diversions(
        "nhmv10_withObsScaled", start_date_training, end_date_training, make_figs=True
    )
    predict_inflows_diversions(
        "nwmv21_withObsScaled", start_date_training, end_date_training, make_figs=True
    )
