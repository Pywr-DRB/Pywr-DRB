...
def reset_pn():
    ...
    try:
        pn.flows.set_all_to_sc(
            mode="folders", prefix="flows/", overwrite=True, exclude=r"^_",
        )
        pn.diversions.set_all_to_sc(
            mode="folders", prefix="diversions/", overwrite=True, exclude=r"^_",
        )
    except TypeError:
        pn.flows.set_all_to_sc(
            only_folders=True, prefix="flows/", overwrite=True, only_exclude=["_*"],
        )
        pn.diversions.set_all_to_sc(
            only_folders=True, prefix="diversions/", overwrite=True, only_exclude=["_*"],
        )
