...
plot_storage = True
plot_links = True
plot_reservoir_types = (
    False  # Allows options to toggle reservoir color by regulation types
)
plot_nhm_hru = True
...
geomap = folium.Map(
    location=start_coords, zoom_start=7.35, tiles="cartodbpositron", control_scale=True
)
...
all_gauges = gpd.GeoDataFrame(
    all_usgs_gauges.drop("geometry", axis=1),
    geometry=gpd.points_from_xy(all_usgs_gauges.long, all_usgs_gauges.lat, crs=crs),
)
...
for i in range(len(all_edges)):
    if (all_edges[i][0] in nodes.name.values) and (
        all_edges[i][1] in nodes.name.values
    ):
...
for _, r in drb_boundary.iterrows():
    ...
    folium.Popup("Delaware River Basin Boundary", min_width=200, max_width=200).add_to(
        geo_j
    )
...
for _, r in drb_river.iterrows():
    ...
    folium.Popup("Delaware River Main Stem", min_width=200, max_width=200).add_to(geo_j)
    geo_j.add_to(basin_layer)
...
if plot_catchments:
    ...
    for _, r in node_basins.iterrows():
        ...
        geo_j = folium.GeoJson(
            data=geo_j,
            style_function=lambda x: {
                "fillColor": basin_colormap(r["area"]),
                "weight": 1.0,
        ...
        folium.Popup(
            f"Catchment basin for {node_name}", min_width=200, max_width=200
        ).add_to(geo_j)
...
for node, sites in obs_site_matches.items():
    if s:
        for s in sites:
            ...
            if len(s_i) > 0:
                coords = [drb_gauges.loc[s_i, "lat"], drb_gauges.loc[s_i, "long"]]
                ...
                pop = folium.Popup(
                    disp_text, min_width=popup_width, max_width=popup_width
                )
...
for l in range(n_edges):
    ...
    folium.PolyLine(
        line, weight=tributary_weight, color=tributary_color, opacity=edge_opacity
    ).add_to(geomap)
...
for n in range(n_nodes):
    ...
    if node_type == "storage":
        ...
        volume = reservoir_data[reservoir_data["reservoir"] == res_name][
            "GRanD_CAP_MCM"
        ].iloc[0]
        ...
        if name == "outflow_delTrenton":
            folium.Marker(
                coords, popup=pop, icon=folium.Icon(color="green"), radius=100
            ).add_to(flow_target_layer)
        elif name == "outflow_delMontague":
            folium.Marker(
                coords, popup=pop, icon=folium.Icon(color="purple"), radius=60
            ).add_to(flow_target_layer)
...
if plot_reservoir_types:
    ...
    consumptive_makeup_reservoir_layer = folium.FeatureGroup(
        name="Consumptive Make-Up", show=False
    )
        ...
        if node_type == "storage":
            ...
            if name in nyc_main:
                ...
                folium.RegularPolygonMarker(
                    ...
                    rotation=90,
                ).add_to(emergency_reservoir_layer)
            elif name in docket:
                folium.RegularPolygonMarker(
                    coords,
                    popup=pop,
                ...
                folium.RegularPolygonMarker(
                    ...
                    rotation=90,
                ).add_to(consumptive_makeup_reservoir_layer)
