...
resops_in = pd.read_csv(
    "DAILY_AV_INFLOW_CUMECS.csv",
    delimiter=",",
    infer_datetime_format=True,
    low_memory=False,
)
resops_out = pd.read_csv(
    "DAILY_AV_OUTFLOW_CUMECS.csv",
    delimiter=",",
    infer_datetime_format=True,
    low_memory=False,
)
resops_storage = pd.read_csv(
    "DAILY_AV_STORAGE_MCM.csv",
    delimiter=",",
    infer_datetime_format=True,
    low_memory=False,
)
    ...
    for res, res_id in zip(reservoirs, reservoir_ids):
        ...
        # Check if more than one reservoir included
            ...
            combined_data = pd.DataFrame(
                {
                    "date": resops_in["date"],
                    "inflow": np.zeros(len(resops_in["date"])),
                    "outflow": np.zeros(len(resops_in["date"])),
                    "storage": np.zeros(len(resops_in["date"])),
                }
            )
            ...
            for sub_id, counter in zip(many_ids, range(len(many_ids))):
                ...
                if sub_id not in resops_out.columns:
                    print(
                        f"{res} with res_id {sub_id} was not found in the ResOps database."
                    )
                    ...
                    sub_data = pd.DataFrame(
                        {
                            "inflow": resops_in[sub_id].values,
                            "outflow": resops_out[sub_id].values,
                            "storage": resops_storage[sub_id].values,
                        }
                    )
        ...
        if res_id not in resops_out.columns:
            ...
            data = pd.DataFrame(
                {
                    "date": resops_in["date"],
                    "inflow": resops_in[res_id].values,
                    "outflow": resops_out[res_id].values,
                    "storage": resops_storage[res_id].values,
                }
            )
