...
class Reservoir:
    def __init__(self, starfit_params, reservoir_name, inflow, S_initial, **kwargs):
        """
        Parameters:
        ----------
    ...
    def parse_starfit_params(self):
        ...
        self.R_max = (
            (self.params["Release_max"] + 1) * self.params["GRanD_MEANFLOW_MGD"]
        ).values
        self.R_min = (
            (self.params["Release_min"] + 1) * self.params["GRanD_MEANFLOW_MGD"]
        ).values
    ...
    def release_harmonic(self, time):
        if self.timestep == "daily":
            time = time / 7
        R_avg_t = (
            self.params["Release_alpha1"] * self.sinNpi(time, 2)
    ...
    def calc_NOR_hi(self, time):
        ...
        NOR_hi = (
            self.params["NORhi_mu"]
            + self.params["NORhi_alpha"] * self.sinNpi(time, 2)
    ...
    def calc_NOR_lo(self, time):
        ...
        NOR_lo = (
            self.params["NORlo_mu"]
            + self.params["NORlo_alpha"] * self.sinNpi(time, 2)
    ...
    def release_adjustment(self, S_hat, time):
        ...
        epsilon = (
            self.params["Release_c"]
            + self.params["Release_p1"] * A_t
