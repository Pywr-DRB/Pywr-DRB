...
def match_gages(df, dataset_label):
    ...
    if dataset_label in ['obs_pub', 'obs']:
        use_pub = True
        df_pub = pd.read_csv(f'{input_dir}modeled_gages/drb_pub_predicted_flows.csv', sep = ',', index_col=0)
...
def extrapolate_NYC_NJ_diversions(loc):
    ...
    if loc == 'nyc':
        ...
        df_long = pd.DataFrame({'flow_log': np.log(flow[trenton_catchments].sum(axis=1)),
                                'm': flow.index.month,
                                'y': flow.index.year,
                                'q': [get_quarter(m) for m in flow.index.month]})
