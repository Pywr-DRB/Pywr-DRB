...
def plot_3part_flows(results, models, node,
    ...
    for i, m in enumerate(models):
        if use2nd or i == 0:
            ...
            nse, kge, r, alpha, beta = round(nse[0], 2), round(kge[0], 2), round(r[0], 2), round(alpha[0], 2), round(
                beta[0], 2)
...
def plot_radial_error_metrics(results_metrics, radial_models, nodes, useNonPep=True, useweap=True, usepywr=True,
    ...
    edgedict = {'obs_pub': 'w', 'nhmv10': 'w', 'nwmv21': 'w', 'nwmv21_withLakes': 'w', 'WEAP_24Apr2023_gridmet': 'w',
                'pywr_obs_pub': 'w', 'pywr_nhmv10': 'w', 'pywr_nwmv21': 'w', 'pywr_nwmv21_withLakes': 'w',
                'pywr_WEAP_24Apr2023_gridmet': 'w'}
...
def plot_rrv_metrics(rrv_metrics, rrv_models, nodes, fig_dir=fig_dir,
    ...
    for n, node in enumerate(nodes):
        for k, metric in enumerate(metrics):
            ...
            heights = [rrv_metrics[metric].loc[
                           np.logical_and(rrv_metrics['node'] == node, rrv_metrics['model'] == model)].iloc[0] for model
                       in rrv_models]
...
def plot_flow_contributions(reservoir_releases, major_flows,
    ...
    if percentage_flow:
        ...
        ax.hlines(target, ts[0], ts[-1], linestyle='dashed', color='black', alpha=0.5,
                  label=f'Flow target {target} (MGD)')
...
def plot_combined_nyc_storage(storages, releases, models,
    ...
    for model in model_names:
        drought_levels[model] = get_pywr_results(output_dir, model, results_set='res_level').loc[start_date:end_date,
                                ['nyc']]
    ...
    ax3.plot(historic_release['FFMP_min_release'], color='black', ls=':',
    ...
    plt.savefig(
        f'{fig_dir}combined_NYC_reservoir_operations_{start_date.strftime("%Y-%m-%d")}_{end_date.strftime("%Y-%m-%d")}.png',
        dpi=250)
    # plt.show()
    return