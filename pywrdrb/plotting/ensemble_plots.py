...
def plot_ensemble_NYC_release_contributions(model,
    ...
    use_contribution_model = model.split('_ensemble')[0] if 'ensemble' in model else model
    ...
    if 'ensemble' in model:
        ...
        for i,real in enumerate(realizations):
            release_total = subset_timeseries(reservoir_releases[model][real][reservoir_list_nyc], 
                                              start_date, end_date).sum(axis=1)
            if i == 0:
                ensemble_downstream_gage_pywr = pd.DataFrame(release_total, columns=[real], index=release_total.index)
            else:
        ...
        ax.fill_between(ensemble_downstream_gage_pywr.index,
                        ...
                        color=colordict[model], alpha=0.5, zorder=2, lw=0.0,
                        label=model_label_dict[model])
                        
           
    ax.plot(downstream_gage_pywr, color='k', lw=1.7, zorder=3)
    ax.plot(downstream_gage_pywr, color=colordict[use_contribution_model], lw=1.4, 
            label=f'{model_label_dict[use_contribution_model]} Flow', zorder=3.1)
    ...
    if plot_observed:
        ...
        if len(downstream_gage_obs) > 0:
            ax.plot(downstream_gage_obs, color='k', ls=':', lw=1.7, 
    ...
    ax_twin.set_yticks(ax_twin.get_yticks(), ax.get_yticklabels(), 
                       fontsize=fontsize)
    return 
...
def plot_ensemble_node_flow_contributions(model,
    ...
    ax_twin.set_yticks(ax_twin.get_yticks(), ax_twin.get_yticklabels(), fontsize=fontsize)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=fontsize)
...
def plot_NYC_release_components_combined(storages, ffmp_level_boundaries, 
    ...
    filename = f'{fig_dir}NYC_release_components_combined_{model}_{node}_' + \
...
def plot_ensemble_nyc_storage(storages, 
    ...
    for m,c in zip(models,line_colors):
        ...
        if 'ensemble' in m:
            ...
            # Plot quantiles            
            if percentiles_cmap:
                cmap = create_mirrored_cmap('Oranges') if 'nhm' in m else create_mirrored_cmap('Blues')
                ...
                for i in range(len(percentiles)-1):
                    ...
                    ax.fill_between(ensemble_modeled_storage.index,
                                    ...
                                    alpha=ensemble_fill_alpha, zorder=2, lw=0.0)
                    ax.fill_between(ensemble_modeled_storage.index,
                                    ensemble_modeled_storage.quantile(1-percentiles[i+1], axis=1),
            ...
            if plot_ensemble_mean:
                ax.plot(ensemble_modeled_storage.mean(axis=1), 
                        color=c, 
                        ls='-', 
                        zorder=4, 
                        lw=1.6)
                ax.plot(ensemble_modeled_storage.mean(axis=1), color='k', ls='-', zorder=3, lw=2)
            
        else:
            modeled_storage = subset_timeseries(storages[m][reservoir_list_nyc], start_date, end_date).sum(axis=1)
            modeled_storage *= 100/capacities['combined']
            modeled_storage = modeled_storage.rolling(smoothing_window, center=True).mean()
            ax.plot(modeled_storage, color='k', ls='-', zorder=5, lw=2)
            ax.plot(modeled_storage, color=c, ls='-', label=model_label_dict[m], zorder=6, lw=1.6)
...
def plot_ensemble_deficit(results, 
                          models, 
                          node, 
                          start_date, 
                          end_date,
    ...
    for model in models:
        ...
        if 'ensemble' in model:
            assert(node in results[model][list(results[model].keys())[0]].columns), f'{node} not in results.'
        else:
    ...
    for model in models:
        if 'ensemble' in model:
            ...
            for i,real in enumerate(ensemble_realizations):
                ...
                if i == 0:        
                    df = pd.DataFrame(percent_target.values, columns=[real], 
            ...
            # Plot ensemble mean
            if plot_ensemble_mean:
                ax.plot(df.mean(axis=1), color=model_colors[model], ls='-', lw=1, zorder=4)
                ax.plot(df.mean(axis=1), color='k', ls='-', lw=1.5, zorder=3)
            # if fill_between_quantiles:

            if percentiles_cmap:
                percentiles = np.linspace(0.01, 0.99, 50)[::-1]
                cmap = create_mirrored_cmap('Oranges') if 'nhm' in model else create_mirrored_cmap('Blues') 
                ...
                ax.fill_between(df.index,
                                ...
                                alpha=fill_alpha, zorder=2, lw=1.6,
                                label = model_label_dict[model])
        else:
            ...
            percent_target = percent_target.rolling(smoothing_window, center=True).mean()
            ax.plot(percent_target, color=model_colors[model], ls='-', lw=1, zorder=6,
                    label=model_label_dict[model])
    ...
    if plot_observed:
        ...
        obs.plot(ax=ax, color='k', linewidth=2, 
                 label=model_label_dict['obs'],
                 ls=':', zorder=10)
    ...
    if not is_subplot:
        ...
        ax.set_xlabel('Year', fontsize=fontsize)
        ax.legend(frameon=False, fontsize=fontsize, loc='upper left',
                  bbox_to_anchor=(0.0, -0.19), ncols=3)
    else:
        return ax
...
def plot_ensemble_node_flow(major_flows,
                            models,
                            node,
                            start_date,
                            end_date,
    ...
    for model in models:
        ...
        if 'ensemble' in model:
            ...
            for i, real in enumerate(realizations):
                ...
                if i == 0:
                    ensemble_flow = pd.DataFrame(modeled_flow, columns=[real], 
                                                 index=modeled_flow.index)
                else:
            ...
            if percentile_cmap:
                ...
                mirrored_cmap = create_mirrored_cmap('Oranges') if 'nhm' in model else create_mirrored_cmap('Blues')
                norm = Normalize(vmin=-0.25, vmax=1.25)
                for i in range(len(percentiles)-1):
                    ax.fill_between(ensemble_flow.index, 
                ...
                ax.fill_between(ensemble_flow.index,
                                ...
                                zorder=2)
            if plot_ensemble_mean:
                ax.plot(ensemble_flow.index, ensemble_flow.mean(axis=1), 
                        color='k', lw=2, zorder=3)
                ax.plot(ensemble_flow.index, ensemble_flow.mean(axis=1), 
                        color=colordict[model], lw=1.5, zorder=4, ls='-')
            ...
            ax.plot(modeled_flow.index, modeled_flow, color=colordict[model], lw=1.5, zorder=4, ls='-')
    ...
    if plot_observed:
        ...
        ax.plot(obs_flow.index, obs_flow, color='k', 
                lw=2, ls=':', zorder=7)
    if plot_target:
        ax.axhline(target, color='maroon', lw=2, 
                   ls='--', zorder=0,
    ...
    if not is_subplot:
        ...
        fig.tight_layout()
        fig.savefig(fig_dir + f'{node}_ensemble_flow_{start_str}_{end_str}.png', dpi=dpi)
...
def plot_ensemble_nyc_storage_and_deficit(storages,
                                          major_flows,
                                            ffmp_level_boundaries,
                                            models,
                                            colordict = model_colors_historic_reconstruction,
    ...
    plot_ensemble_nyc_storage(storages,
                                ...
                                plot_ensemble_mean=plot_ensemble_mean,
                                ensemble_fill_alpha=ensemble_fill_alpha,
                                dpi=dpi, legend=False)
    
    
    ### subplot b: deficits at Montague
    plot_ensemble_deficit(major_flows,
                              models = models,
                                node = 'delMontague',
                                start_date = start_date,
                                end_date = end_date,
    ...
    ax3.legend(handles, labels, frameon=False, fontsize=fontsize, loc='upper center',
                bbox_to_anchor=(0.5, -0.5), ncols=3)
...
def plot_ensemble_nyc_storage_flow_deficit(storages,
                                          major_flows,
                                            ffmp_level_boundaries,
                                            models,
                                            node,
    ...
    plot_ensemble_node_flow(major_flows,
                          models=models,
                            node=node,
                            start_date=start_date,
                            end_date=end_date,
    ...
    plot_ensemble_deficit(major_flows,
                              models = models,
                                ...
                                fill_alpha=ensemble_fill_alpha,
                                dpi=dpi)    
    ylabel = f'Montague Target\nSatisfied (%)' if node == 'delMontague' else f'Trenton Target\nSatisfied (%)'
