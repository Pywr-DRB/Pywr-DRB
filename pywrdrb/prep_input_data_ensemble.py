...
from utils.hdf5 import get_hdf5_realization_numbers, export_ensemble_to_hdf5, extract_realization_from_hdf5
...
if __name__ == "__main__":
    ...
    for dataset in ensemble_datasets:
        ...
        if rank == 0:
            print(f'Starting inflow & diversion predictions for {dataset}')
    
            if 'syn' in dataset:
                ensemble_input_dir = input_dir + 'synthetic_ensembles/'
            elif 'obs_pub' in dataset:
                ensemble_input_dir = input_dir + 'historic_ensembles/'
            
            ensemble_filename = f'{ensemble_input_dir}catchment_inflow_{dataset}.hdf5'
            ...
            test_realization = extract_realization_from_hdf5(ensemble_filename, realization_numbers[0], 
                                                             stored_by_node=True)
        ...
        for i, real in enumerate(proc_realizations):
            ...
            df_predictions = predict_inflows_diversions(dataset, start_date, end_date,
                                                        use_log=True, remove_zeros=False, 
                                                        use_const=False,
                                                        ensemble_inflows=True, realization=str(real),
                                                        save_predictions=False, return_predictions=True,
                                                        make_figs=False,
                                                        catchment_inflows=None)
            
            df_predictions = df_predictions.drop(columns=[dt for dt in ['date', 'datetime'] if dt in df_predictions.columns])
        ...
        comm.Gatherv(ensemble_predictions, 
                     (gathered_data, sizes, displacements, MPI.DOUBLE), root=0)
        ...
        if rank == 0:
            ...
            # setup 3d array 
            ensemble_predictions_mat = np.empty((n_realizations, len(datetime), len(df_columns)))
            ...
            for rank_i in range(comm.size):
                ...
                for rank_real_ij in range(n_reals_on_ri):
                    if rank_i == 0:
                        ...
                        real_idx = np.cumsum(n_realizations_per_proc[:rank_i])[-1] + rank_real_ij
                    ...
                    ensemble_predictions_mat[real_idx, :, :] = gathered_data[start_idx:end_idx].reshape(len(datetime), len(df_columns))
                    start_idx = end_idx
            ...
            output_filename = f'{input_dir}predicted_inflows_diversions_{dataset}.hdf5'
            print(f'Exporting {dataset} predicted inflows/diversions to hdf5.')
            export_ensemble_to_hdf5(combined_ensemble_pred, output_filename)
        
            print(f'Done preparing ensemble data for {dataset}!')
        ...
        # # export local process data to hdf5
        # local_filename = f'{input_dir}predicted_inflows_diversions_{dataset}_rank{rank}.hdf5'
        # export_ensemble_to_hdf5(ensemble_pred_nonnyc_gage_flows, local_filename)
        
        # # Gather local filenames
        # all_filenames = comm.gather(local_filename, root=0)
        
        # # Combine all local files into a single file
        # if rank == 0:
        #     combined_output_filename = f'{input_dir}predicted_inflows_diversions_{dataset}.hdf5'
            
        #     combine_batched_hdf5_outputs(batch_files=all_filenames, 
        #                                  combined_output_file=combined_output_filename)

        #     # Delete local files
        #     for file in all_filenames:
        #         os.remove(file)
        #     print('Done preparing ensemble data!')
            
            
            
        # Put local data into np array
    #     local_data = np.array((n_realizations_per_proc, ))

    #     gathered_data = None
    #     if rank == 0:
    #         gathered_data = np.empty((n_realizations, ))
    ...
    #         all_ensemble_pred = [pickle.loads(data) for data in all_pickle_data]
    #         combined_ensemble_pred = {key: val for d in all_ensemble_pred for key, val in d.items()}
            
    #         datetime = pd.date_range(start=start_date, end=end_date, freq='D')
    #         datetime = datetime.astype(str).tolist()
    #         for key in combined_ensemble_pred.keys():
    #             combined_ensemble_pred[key]['date'] = datetime
    #             combined_ensemble_pred[key]['datetime'] = datetime
