...
def read_usgs_elev_txt(file_path):
    ...
    df = df.drop(0, axis=0)
    df = df.rename(columns={"14n": res_name, "20d": "datetime"})
...
if __name__ == "__main__":
    # Load elevation data
    beltzville = read_usgs_elev_txt(
        f"{input_dir}historic_reservoir_ops/beltzvilleCombined_01449790_elevation.txt"
    )
    blueMarsh = read_usgs_elev_txt(
        f"{input_dir}historic_reservoir_ops/blueMarsh_01470870_elevation.txt"
    )
    fewalter = read_usgs_elev_txt(
        f"{input_dir}historic_reservoir_ops/fewalter_01447780_elevation.txt"
    )

    # Concat and save as single CSV
    all_reservoirs = pd.concat([beltzville, blueMarsh, fewalter])
    all_reservoirs.to_csv(
